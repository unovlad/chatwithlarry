...
export class BumpySkiesService {
  ...
  async createDetailedTurbulenceForecast(flightRoute: FlightRoute): Promise<BumpySkiesForecast> {
    console.log(`🌪️ Creating BumpySkies-style forecast for ${flightRoute.flightNumber}`);
    ...
    const turbulenceSegments = this.generateTurbulenceSegments(routeSegments, weatherData);
    
    // 4. Format as BumpySkies-style output
    return this.formatBumpySkiesOutput(flightRoute, turbulenceSegments);
  }

  private createDetailedRouteSegments(flightRoute: FlightRoute): DetailedRouteSegment[] {
    console.log(`🗺️ Creating detailed route segments for ${flightRoute.from.iata} → ${flightRoute.to.iata}`);
  ...
  private async getWeatherDataForRoute(segments: DetailedRouteSegment[]): Promise<WeatherData[]> {
    console.log(`🌦️ Fetching weather data for ${segments.length} route segments`);
    ...
    for (const segment of segments) {
      try {
        ...
        console.warn(`⚠️ Weather data unavailable for segment ${segment.id}:`, error);
  ...
  private generateTurbulenceSegments(
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      const duration = this.calculateConditionDuration(segment, weather, i, segments.length);
  ...
  private formatBumpySkiesOutput(
    ...
    // Add each segment
    for (const segment of segments) {
      const durationText = segment.duration === 1 ? "1 minute" : `${segment.duration} minutes`;
      
      if (segment.condition === "calm") {
        if (segment === segments[segments.length - 1]) {
          formattedForecast.push(`${this.formatTime(segment.startTime)}: Calm skies for the rest of the flight.`);
        } else {
          formattedForecast.push(`${this.formatTime(segment.startTime)}: Calm skies for the next ${durationText}.`);
        }
      } else {
        formattedForecast.push(`${this.formatTime(segment.startTime)}: ${segment.description} for the next ${durationText}.`);
      }
    }
  ...
  private estimateFlightDuration(flightRoute: FlightRoute): number {
    ...
    const distance = this.calculateDistance(
      { lat: flightRoute.from.coordinates?.lat || 0, lon: flightRoute.from.coordinates?.lon || 0 },
      { lat: flightRoute.to.coordinates?.lat || 0, lon: flightRoute.to.coordinates?.lon || 0 }
    );
  ...
  private generateWaypoints(flightRoute: FlightRoute, count: number): Waypoint[] {
    ...
    for (let i = 0; i <= count; i++) {
      ...
      waypoints.push({
        ...
        name: i === 0 ? flightRoute.from.iata : i === count ? flightRoute.to.iata : `Waypoint ${i}`,
      });
    }
  ...
  private calculateAltitude(segmentIndex: number, totalSegments: number): number {
  ...
  private async getStabilityData(segment: DetailedRouteSegment): Promise<StabilityData> {
  ...
  private async getTurbulenceIndices(segment: DetailedRouteSegment): Promise<TurbulenceData> {
    // In a real implementation, this would call NOAA turbulence models
    const index = Math.random();
    return {
      index,
      category: index < 0.3 ? 'smooth' : index < 0.6 ? 'light' : index < 0.9 ? 'moderate' : 'severe',
    };
  }

  private calculateTurbulenceLevel(weather: WeatherData): "calm" | "light" | "moderate" | "severe" {
  ...
  private getTurbulenceDescription(condition: string, duration: number): string {
    const durationText = duration === 1 ? "1 minute" : `${duration} minutes`;
    
    switch (condition) {
      case "calm": return "Calm skies";
      case "light": return "Light turbulence likely";
      case "moderate": return "Moderate turbulence likely";
      case "severe": return "Severe turbulence likely";
